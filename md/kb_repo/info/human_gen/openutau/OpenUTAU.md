...
左侧轨道栏写着 `[Missing] DEFAULT`
意思：**这条轨道没有绑定任何歌手声库**，所以无法渲染人声，教程里第三步你没做对。

## 二、匹配你当前界面的精准操作（一步步跟着点）

### 步骤 1：确认歌手已经安装

顶部菜单 → **工具 (Tools)** → **歌手管理器 (Singers)**

- 列表里有你下载安装好的 DiffSinger/UTAU 歌手才算成功；
- 如果是空的，说明声库没正确安装，需要用工具→安装歌手，导入音源压缩包。

### 步骤 2：给当前主唱轨道绑定歌手（解决 \[Missing\]）

1. 看左侧轨道栏 Track1 右边有个**齿轮⚙️图标**（截图里轨道条右侧），点这个齿轮；
2. 弹出轨道设置窗口，第一栏「歌手」下拉菜单，选中你装好的人声（比如 MUTA 男 / 夏叶子等）；
3. 确定，此时左侧 `[Missing]` 会变成歌手名字，绑定完成。

### 步骤 3：检查歌词（关键！MIDI 转 UST 后必须填字才能出声）

你右侧粉色块是音符，但格子里**空白无汉字歌词**，不填歌词渲染出来只有静音 / 噪音：

1. 双击粉色音符块，下方会出现歌词输入栏；
2. 按音符逐字填入对应中文歌词，每个音符对应一个汉字；
3. 批量填充技巧：顶部工具 → 歌词批量导入，可导入文本一键分配音符。
...
1. 快捷按钮：界面上方播放控件旁边，**摄像机样式的 Render 渲染图标**；
2. 菜单路径：顶部 `工具(Tools)` → `渲染(Render)`；
3. 弹窗设置：
   - 输出路径：可自定义，默认 OpenUtau 安装目录下 `render/`
   - 采样率 44100/48000，立体声，直接确认开始渲染。

### 步骤 5：找到成品文件

渲染完成弹窗提示成功，打开 OpenUtau 根目录 `render` 文件夹，里面就是纯人声干声 WAV。
