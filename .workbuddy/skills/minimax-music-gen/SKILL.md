...
description: >
  Use when user wants to generate music using mmx CLI with MiniMax API.
  Triggers on requests explicitly mentioning "mmx", "CLI", "命令行", "API", or when
  user wants programmatic/batch music generation. This is the mmx CLI wrapper skill.
  For web-based generation, use "minimax-music-web".
  For API reference only, use "minimax-music-api".
license: MIT
metadata:
  version: "1.1"
  category: creative
