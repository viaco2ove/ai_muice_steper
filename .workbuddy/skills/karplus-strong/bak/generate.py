...
输出: 
    1. workspace/project/{song}/song_engineer/track/ karplus/{track_id}.json
    2. workspace/project/{song}/song_engineer/track/ karplus/{track_id}.wav
"""
...
# 默认配置
DEFAULT_MODEL = os.environ.get(' karplus', ' karplus-stereo-melody')
BPM = 68
SAMPLE_RATE = 44100
VOLUME = 0.4
...
def load_track_json(song, track_id):
    ...
    # 排除  karplus 目录下的
    json_files = [f for f in json_files if ' karplus' not in str(f)]
...
def main():
    parser = argparse.ArgumentParser(description=' karplus 音频生成器')
    parser.add_argument('song', help='歌曲名 (如 走在)')
    parser.add_argument('track_id', help='轨道ID (如 08_节奏吉他)')
    parser.add_argument('--model', '-m', default=DEFAULT_MODEL, help=f' karplus 模型 (默认: {DEFAULT_MODEL})')
    ...
    base_path = json_path.parent
     karplus_dir = base_path / ' karplus'
     karplus_dir.mkdir(exist_ok=True)
    
    # 构造 track_id (去掉 .json)
    track_id_clean = args.track_id.replace('.json', '')
    output_json_path =  karplus_dir / f'{track_id_clean}.json'
    output_wav_path =  karplus_dir / f'{track_id_clean}.wav'
